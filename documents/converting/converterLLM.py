...
# Function to prepare bulk data for OpenSearch in the desired format
def prepare_bulk_data(data):
    bulk_data = ""
    unique_id_counter = {}

    for item in data:
        # Create a unique ID, handle duplicates
        base_id = item.get("FKArtikelID")
        if base_id in unique_id_counter:
            unique_id_counter[base_id] += 1
            unique_id = f"{base_id}-{unique_id_counter[base_id]}"
        else:
            unique_id_counter[base_id] = 1
            unique_id = base_id

        # Add the index action
        action = {"index": {"_index": "eisbaer_rag_data", "_id": unique_id}}
        bulk_data += json.dumps(action) + "\n"

        # Create the text field with desired concatenation
        text = f"{item.get('Bezeichnung1', '')}, Stellplatz: {item.get('Stellplatz', '')}"
        document = {"text": text}
...
def main():
    # Provided file paths
    input_sql_file = './insert.sql'
    output_bulk_file = './bulk_data_llm'
